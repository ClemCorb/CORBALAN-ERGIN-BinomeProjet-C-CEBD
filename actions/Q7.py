...
class Window(tk.Toplevel):
    ...
    def modifierMain(self):
        ttk.Label(self, text='Veuillez indiquer le type de travaux :', anchor="center", font=('Helvetica', '10', 'bold')).grid(row=1, column=3)    
        choix=["Photovoltaïque","Chauffage","Isolation"] 
        self.input = ttk.Combobox(self, values=choix, state="readonly")
        self.input.current(0)
        self.input.bind('<<ComboboxSelected>>', self.modifier)
        self.input.grid(row=1, column=4)
        
    def suprimerMain(self):
        ttk.Label(self, text='Veuillez indiquer le type de travaux :', anchor="center", font=('Helvetica', '10', 'bold')).grid(row=1, column=3)    
        choix=["Photovoltaïque","Chauffage","Isolation"] 
        self.input = ttk.Combobox(self, values=choix, state="readonly")
        self.input.current(0)
        self.input.bind('<<ComboboxSelected>>', self.suprimer)
        self.input.grid(row=1, column=4)
    ...
    def ajout(self, event):
        ...
        ttk.Label(self, text='cout_induit_ht:', anchor="center", font=('Helvetica', '10')).grid(row=3, column=3)
        self.cout_induit_ht_entry = ttk.Entry(self)
        self.cout_induit_ht_entry.grid(row=4, column=3)
        labels.append(self.cout_induit_ht_entry)

        ttk.Label(self, text='annee_travaux:', anchor="center", font=('Helvetica', '10')).grid(row=3, column=4)
        self.annee_travaux_entry = ttk.Entry(self)
        self.annee_travaux_entry.grid(row=4, column=4)
        labels.append(self.annee_travaux_entry)

        ttk.Label(self, text='code_departement:', anchor="center", font=('Helvetica', '10')).grid(row=3, column=5)
        self.code_departement_entry = ttk.Entry(self)
        self.code_departement_entry.grid(row=4, column=5)
        labels.append(self.code_departement_entry)

        ttk.Label(self, text='code_region:', anchor="center", font=('Helvetica', '10')).grid(row=3, column=6)
        self.code_region_entry = ttk.Entry(self)
        self.code_region_entry.grid(row=4, column=6)
        labels.append(self.code_region_entry)

        ttk.Label(self, text='type_logement:', anchor="center", font=('Helvetica', '10')).grid(row=3, column=7)
        self.type_logement_entry = ttk.Entry(self)
        self.type_logement_entry.grid(row=4, column=7)
        labels.append(self.type_logement_entry)

        ttk.Label(self, text='anne_construction_logement:', anchor="center", font=('Helvetica', '10')).grid(row=3, column=8)
        self.anne_construction_logement_entry = ttk.Entry(self)
        self.anne_construction_logement_entry.grid(row=4, column=8)
        labels.append(self.anne_construction_logement_entry)
    
        if(event.widget.get()=="Photovoltaïque"):
            #('puissance_installee','type_panneaux')
            ttk.Label(self, text='puissance_installee:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=0)
            ...
            ttk.Label(self, text='type_panneaux:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=1)
        ...
        elif(event.widget.get()=="Chauffage"):
            #('énergie_avant_travaux','energie_installee','generateur','type_chaudiere')
            ttk.Label(self, text='énergie_avant_travaux:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=0)
            ...
            ttk.Label(self, text='energie_installee:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=1)
            ...
            ttk.Label(self, text='generateur:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=2)
            self.generateur_entry = ttk.Entry(self)
            self.generateur_entry.grid(row=6, column=2)
            labels.append(self.generateur_entry)
            
            ttk.Label(self, text='type_chaudiere:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=3)
        ...
        elif(event.widget.get()=="Isolation"):
            #('poste','type_isolant','epaisseur_isolant','surface')
            ttk.Label(self, text='poste_isolation:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=0)
            self.poste_entry = ttk.Entry(self)
            self.poste_entry.grid(row=6, column=0)
            labels.append(self.poste_entry)
            
            ttk.Label(self, text='type_isolant:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=1)
            self.type_isolant_entry = ttk.Entry(self)
            self.type_isolant_entry.grid(row=6, column=1)
            labels.append(self.type_isolant_entry)
            
            ttk.Label(self, text='epaisseur:', anchor="center", font=('Helvetica', '10')).grid(row=5, column=0)
